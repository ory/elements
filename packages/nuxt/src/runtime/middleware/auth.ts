...
import {
  createClientSideFrontendClient,
...
export default defineNuxtRouteMiddleware(async (to) => {
  // During hydration of a server-rendered page the session was already
  // validated on the server; only skip in that exact case. Every real
  // client-side navigation must re-check the session, otherwise SPA
  // navigation would bypass authentication entirely.
  ...
  try {
    if (import.meta.server) {
      ...
      await createFrontendClient().toSession({ cookie: headers.cookie })
    } else {
      // Same-origin request through the Nitro proxy; the browser attaches
      // the session cookie itself.
      await createClientSideFrontendClient().toSession()
    }
