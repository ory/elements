...
/**
 * Create a browser-side Frontend API client.
 *
 * Requests target the app origin so they go through the Nitro proxy routes
 * registered by the module (`/sessions/**` etc.), and credentials are
 * included so the session cookie is sent.
 */
export function createClientSideFrontendClient(): FrontendApi {
  return new FrontendApi(
    new Configuration({
      headers: {
        Accept: "application/json",
