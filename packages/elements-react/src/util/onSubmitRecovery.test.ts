...
import { onSubmitRecovery } from "./onSubmitRecovery"
...
describe("onSubmitRecovery", () => {
  test("should fire recovery success event on successful submission", async () => {
    ...
    mockConfig.sdk.frontend.updateRecoveryFlowRaw.mockResolvedValue({
      value: () => Promise.resolve(successFlow),
    })

    await onSubmitRecovery(
      mockFlow,
  ...
  test("should fire validation error event on validation failure", async () => {
    ...
    const errorFlow = {
      ...
      ui: {
        ...
        nodes: [
          {
            messages: [
              { id: 4000002, text: "Email is required", type: "error" },
            ],
            attributes: {},
            type: "input",
            group: "default",
        ...
        messages: [{ id: 4000001, text: "Account not found", type: "error" }],
      },
    }
    ...
    mockConfig.sdk.frontend.updateRecoveryFlowRaw.mockRejectedValue(error)

    await onSubmitRecovery(
      mockFlow,
      mockConfig as unknown as OryElementsConfiguration,
      {
        body: { method: "code", email: "unknown@example.com" } as any,
        onRedirect,
        setFlowContainer,
        onValidationError,
      },
